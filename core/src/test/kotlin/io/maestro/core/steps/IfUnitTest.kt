...
class IfUnitTest {
    ...
    fun `execute should run ifTrue branch when parameter condition is true`() {
        ...
        val context = ExecutionContext(inputParameters = mapOf("enabled" to true))
    ...
    fun `execute should run ifFalse branch when parameter condition is false`() {
        ...
        val context = ExecutionContext(inputParameters = mapOf("enabled" to false))
    ...
    fun `execute should evaluate equality condition against context parameters`() {
        ...
        val context = ExecutionContext(inputParameters = mapOf("environment" to "production"))
    ...
    fun `execute should treat string values as truthy or falsy`() {
        ...
        for (value in truthyValues) {
            ...
            val context = ExecutionContext(inputParameters = mapOf("flag" to value))
            val (status, resultContext) = ifStep.execute(context)
    ...
    fun `execute should treat numeric zero as falsy`() {
        ...
        // Test zero
        val contextZero = ExecutionContext(inputParameters = mapOf("count" to 0))
        ...
        // Test non-zero
        val contextNonZero = ExecutionContext(inputParameters = mapOf("count" to 5))
